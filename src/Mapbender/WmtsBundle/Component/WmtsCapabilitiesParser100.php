...
//            }
//            /* parse wmts:_ExtendedOperation  */ else if ($capabilityEl->localName ===
...
//        }
    ...
    {
        #$wmts->setName($this->getValue("./ows:Name/text()", $contextElm));
        ...
        foreach ($keywordElList as $keywordEl) {
            ...
            $keyword->setSourceclass($wmts->getClassname());
            $keyword->setSourceid($wmts);
        ...
        #$wmts->setServiceTypeVersion($this->getValue("./ows:ServiceTypeVersion/text()", $contextElm));
        #s. $wmts->setVersion($this->getValue("./@version", $root));
    ...
    private function parseServiceProvider(WmtsSource $wmts, \DOMElement $contextElm)
    {
        ...
        $contact->setVoiceTelephone($this->getValue(
            "./ows:ServiceContact/ows:ContactInfo/ows:Phone/ows:Voice/text()", $contextElm));
        $contact->setFacsimileTelephone($this->getValue(
            "./ows:ServiceContact/ows:ContactInfo/ows:Phone/ows:Facsimile/text()", $contextElm));
        $contact->setAddress($this->getValue(
                "./wmts:ContactInformation/wmts:ContactAddress/wmts:Address/text()", $contextElm));
        $contact->setAddressCity($this->getValue(
                "./ows:ServiceContact/ows:ContactInfo/ows:Address/ows:DeliveryPoint/text()", $contextElm));
        $contact->setAddressStateOrProvince($this->getValue(
                "./ows:ServiceContact/ows:ContactInfo/ows:Address/ows:AdministrativeArea/text()", $contextElm));
        $contact->setAddressPostCode($this->getValue(
            "./ows:ServiceContact/ows:ContactInfo/ows:Address/ows:PostalCode/text()", $contextElm));
        $contact->setAddressCountry($this->getValue(
            "./ows:ServiceContact/ows:ContactInfo/ows:Address/ows:Country/text()", $contextElm));
        $contact->setElectronicMailAddress($this->getValue(
    ...
    private function parseOperationRequestInformation(\DOMElement $contextElm)
    {
        ...
        $requestImformation->setHttpGet($this->getValue(
                "./wmts:DCPType/wmts:HTTP/wmts:Get/wmts:OnlineResource/@xlink:href", $contextElm));
        $requestImformation->setHttpPost($this->getValue(
                "./wmts:DCPType/wmts:HTTP/wmts:Post/wmts:OnlineResource/@xlink:href", $contextElm));
